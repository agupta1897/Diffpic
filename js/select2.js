...
//  }
    ...
    repoSelect.on('select2:select', function (e) {
        ...
        $.ajax({
            url: "https://api.github.com/repos/" + $("#username-field").val() + "/" + data["text"] + "/commits?"+ "access_token=" + access_tokenGlobal +"&scope=repo&token_type=bearer",
            jsonp: true,
            method: "GET",
            dataType: "json",
            success: function(res) {
                ...
                var sha = res[0]["sha"];
                $.ajax({
                    url: "https://api.github.com/repos/" + $("#username-field").val() + "/" + data["text"] + "/git/trees/" + sha + "?recursive=1" + "&access_token=" + access_tokenGlobal +"&scope=repo&token_type=bearer",
                    jsonp: true,
                    method: "GET",
                    dataType: "json",
                    success: function(res) {
